...
def run_json_command(env: dict[str, str], script: Path, *args: str) -> dict:
    output = subprocess.check_output(["python3", str(script), *args], text=True, env=env)
...
def main() -> int:
    ...
    check("bind stores project_root", res["project_root"] == "/private/tmp/project-a")

    show0 = rt.show()
    check("show exposes full state", "state" in show0)
    check("default posture is high_quality", show0["state"]["contract"]["delivery_posture"] == "high_quality")
    ...
    check("readiness is ok after contract+thinking+next_action", show["readiness"]["ok"] is True)

    blocked_activate = rt.activate()
    check("activate blocked without authorization", blocked_activate["action"] == "activate_blocked")
    ...
    check("ignored-only update explains accepted keys", "update only accepts contract, thinking, and progress patches." in noop_update["warnings"])
    ...
    check("update rebinding project_root works", update_result["project_root"] == "/private/tmp/root-two")
    ...
    check("cli update persists objective", cli_show_state["contract"]["objective"] == "Build the first production slice")
    check("cli update persists why_now", cli_show_state["contract"]["why_now"] == "The user approved the scoped plan.")
    check("cli update persists guardrails", cli_show_state["contract"]["guardrails"] == ["Do not bypass reviewable checkpoints."])
    ...
    check("cli checkpoint updates latest_checkpoint", cli_show_after_checkpoint["progress"]["latest_checkpoint"] == "Completed the first implementation review.")
    ...
    check("cli return-to-inc clears authorization", run_cli(env, "show")["state"]["activation"]["status"] == "idle")

    cli_close = run_cli(env, "close", "--reason", "completed", "--summary", "Closed after review.")
    check("cli close disables session", cli_close["mode"] == "disabled")

    alias_show = run_alias(env, "show")
    check("orchestrator alias still works", alias_show["state"]["runtime"]["mode"] == "disabled")

    # 4. Hook behavior through the real stdin protocol
    active_env = set_env(tempfile.mkdtemp(prefix="llr-active-hook-test-"))
    active_cur = run_cli(active_env, "current", "--auto-create", "--project-root", "/tmp/active-hook-project")
    run_cli(
        active_env,
        "update",
        "--json",
    ...
    check("state_context_hook injects in active", active_hook["hookSpecificOutput"]["hookEventName"] == "UserPromptSubmit")
    check("active hook message mentions active objective", "active long-long-run objective" in active_hook["hookSpecificOutput"]["additionalContext"])
    ...
    inc_cur = run_cli(inc_env, "current", "--auto-create", "--project-root", "/tmp/inc-project")
    ...
    check("inc hook injects context", inc_hook["hookSpecificOutput"]["hookEventName"] == "UserPromptSubmit")
    ...
    (legacy_dir / f"{legacy_session}.json").write_text(json.dumps(legacy_state), encoding="utf-8")
    ...
    check("legacy show explains no auto-migration", "does not auto-migrate old long-long-run state files" in legacy_show["error"])
    ...
    check("broken hook injects repair guidance", "could not be loaded" in broken_hook["hookSpecificOutput"]["additionalContext"])
